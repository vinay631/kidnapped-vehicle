	...
	for (int i = 0; i < num_particles; i++) {
		...
		if (fabs(yaw_rate) < 0.0001) {
			...
			new_x_mean = particles[i].x
					+ (velocity / yaw_rate)
							* (sin(new_theta) - sin(particles[i].theta));
			new_y_mean = particles[i].y
					+ (velocity / yaw_rate)
							* (cos(particles[i].theta) - cos(new_theta));
		}
...
void ParticleFilter::updateWeights(double sensor_range, double std_landmark[],
	...
	for (int i = 0; i < int(particles.size()); i++) {
		...
		for (int k = 0; k < int(observations.size()); k++) {
			//Convert measurements to map coordinate
			...
			double e = calculateWeights(converted_obs, best_landmark,
					std_landmark);
...
LandmarkObs ParticleFilter::associateLandmark(LandmarkObs converted_obs,
		Map map_landmarks, double std_landmark[]) {
	LandmarkObs best_landmark;
...
double ParticleFilter::calculateWeights(LandmarkObs obs,
		LandmarkObs best_landmark, double std_landmark[]) {
...
string ParticleFilter::getAssociations(Particle best) {
	...
	stringstream ss = copyToStringStream(v);
	string s = ss.str();
	return replaceTrailingSpace(s);
}

stringstream ParticleFilter::copyToStringStream(vector<int> v) {
	stringstream ss;
	copy(v.begin(), v.end(), ostream_iterator<int>(ss, " "));
	return ss;
}

stringstream ParticleFilter::copyToStringStream(vector<double> v) {
	stringstream ss;
	copy(v.begin(), v.end(), ostream_iterator<float>(ss, " "));
	return ss;
}

string ParticleFilter::replaceTrailingSpace(string s) {
